...
# GRAPHIC 1: BEVERIDGE CURVE WITH TRENDLINE

BC_q <- jolts %>%  filter(series_id %in% c("JTS000000000000000QUR")) %>%
  mutate(value = value/100, urate = (urate/100)) %>%
  filter(year<2020) %>% select(date, value, urate)

BC_q <- lm(value ~ urate*urate, data=BC_q)

jolts %>% filter(period != "M13", seasonal == "S", ratelevel_code == "R") %>%
  filter(series_id %in% c("JTS000000000000000QUR")) %>%
  mutate(last_date = if_else(date >= max(date) %m-% months(month_delay),date,as_date(NA))) %>%
  mutate(decade = if_else(year >= 2020, "2020s", "2000s-2010s")) %>%
  ...
  labs(title="The Beveridge Curve Using Quits is Returning to Trend",
       subtitle="Total nonfarm quit rate verus unemployment rate",
  ...
  theme(axis.text.x = element_text(size=15, face="bold"),
        ...
        axis.title.x = element_text(size=15, family="Larsseit")
  ) +
  theme(panel.grid.minor = element_blank()) + theme(panel.grid.major = element_blank()) +
  geom_text_repel(aes(label = format(last_date, format = "%b \n%y")),color="skyblue", segment.color = 'skyblue', min.segment.length = 0) +
  theme(legend.position = "bottom")

ggsave("graphics/jolts1_bc_quits.png", width = 12, height=6.75, dpi="retina")
...
jolts %>% filter(period != "M13", seasonal == "S", ratelevel_code == "R") %>%
  ...
  mutate(value = value/100, urate = urate/100) %>%
  ggplot(aes(x=urate, y=value, color=decade)) + geom_point() + geom_path() +
  labs(title="The Beveridge Curve Using Quits",
       subtitle="Total nonfarm quit rate verus unemployment rate",
       x="Unemployment Rate", y="",
...
a %>% filter(year == 2022) %>% left_join(b, by="industry_text") %>% rename(v2022 = value) %>%
  ...
  labs(title="Quits are falling across industries in proportion to how they rose",
       subtitle="Change in Quit Rate, 12/2019 to 12/21, and 12/21 to 10/22",
